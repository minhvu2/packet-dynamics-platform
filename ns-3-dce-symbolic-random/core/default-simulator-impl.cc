...
void
DefaultSimulatorImpl::SetNumberSymPackets (uint32_t numSympackets)
{
  ListScheduler::SetNumberSymPackets (numSympackets);	
}

void
DefaultSimulatorImpl::SetFirstSymPacket (uint32_t firstSymPacket)
{
...
// <M>
//  if (s2e_is_symbolic (&m_currentTs, sizeof (uint64_t)))
//    {
//      s2e_concretize (&m_currentTs, sizeof (uint64_t));
//    }
// <M>
...
DefaultSimulatorImpl::Run (void)
{
  ...
  // <M> print delay for all packets
  ListScheduler::PrintPacketInfo ();
  // <M>
...
// <M>
  uint64_t tmp = m_currentTs;
  if (s2e_is_symbolic (&tmp, sizeof (uint64_t)))
    {
      s2e_get_example (&tmp, sizeof (uint64_t));
    }
