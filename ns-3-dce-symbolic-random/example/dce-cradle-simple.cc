...
uint64_t m_interval = 1024000000;
uint32_t m_numSymPackets = 1;
uint32_t m_firstSymPacket = 0;
uint32_t m_maxBytes = 1800;
...
main (int argc, char *argv[])
{
  ...
  cmd.AddValue ("interval", "symbolic interval", m_interval);
  cmd.AddValue ("num", "number of symbolic packets", m_numSymPackets);
  cmd.AddValue ("first", "first symbolic packet", m_firstSymPacket);
  cmd.AddValue ("maxBytes", "max bytes to send", m_maxBytes);
  cmd.Parse (argc, argv);
  
  Simulator::SetInterval (m_interval);
  Simulator::SetNumberSymPackets (m_numSymPackets);
  Simulator::SetFirstSymPacket (m_firstSymPacket);
